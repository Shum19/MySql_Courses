USE lesson_2;
DROP TABLE clients, users, posts;
...
-- в этом задании не могу понять почему post_id имеет значение null?
SELECT post_id FROM clients;
desc clients;
...
SELECT clients.id, clients.login, posts.title
FROM clients, posts 
WHERE posts.id = clients.id;


-- №3.	Выполните поиск  по публикациям, автором котоырх является клиент "Mikle".
SELECT clients.login, posts.title, posts.full_text
FROM clients, posts
WHERE login = 'Mikle' AND posts.id = clients.id;